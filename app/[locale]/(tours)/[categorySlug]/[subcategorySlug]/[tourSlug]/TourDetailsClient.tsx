...
const labelCls =
  "block text-[11px] font-semibold text-gray-500 mb-1.5 uppercase tracking-wide";
...
export default function TourDetailsClient({ tour }: TourDetailsClientProps) {
  ...
  const fpCheckIn = useRef<{
    destroy(): void;
    setDate(d: string, t: boolean): void;
  } | null>(null);
  const fpCheckOut = useRef<{
    destroy(): void;
  ...
  /* ── Dynamic API Data ── */
  const tourImages = (
    tour.images?.filter(Boolean)?.length
      ? tour.images
  ...
  const priceTable: PriceRow[] = (() => {
    ...
    if (tour.pricingTables?.length && tour.pricingTables[0].rows.length > 0) {
      return tour.pricingTables[0].rows.map((r) => ({
        category: r.category
          ? `${r.category} ${r.category === "1" ? "Person" : "Persons"}`
          : "Standard",
        price: r.price,
      }));
    }
  ...
  const handleCounter = useCallback(
    (field: "adults" | "children", inc: boolean) => {
      setFormData((prev) => {
  ...
  const handleChildAgeChange = useCallback((index: number, value: string) => {
    ...
    setFieldErrors((p) => ({
      ...p,
      [`childAges_${index}`]: undefined,
      childAges: undefined,
    }));
  }, []);

  const handleChildAgeBlur = useCallback(
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.ok) {
      ...
      toast.success("Booking Request Sent! 🎉", {
        description:
          result.message || "Our team will contact you within 24 hours.",
        duration: 6000,
      });
  ...
  const navigateLightbox = (dir: "prev" | "next") =>
    setSelectedImageIndex((p) =>
      dir === "prev"
        ? p === 0
          ? safeTourImages.length - 1
          : p - 1
        : p === safeTourImages.length - 1
          ? 0
          : p + 1,
    );
              ...
              <div>
                <div className="flex flex-wrap items-center gap-3 mb-4">
                  ...
                  <span className="inline-flex items-center gap-1.5 text-gray-500 text-sm">
                    <MapPin className="w-4 h-4 text-[var(--second-color)]" />{" "}
                    {locationText}
                  </span>
                  <span className="inline-flex items-center gap-1.5 text-gray-500 text-sm">
                    <Clock className="w-4 h-4 text-[var(--second-color)]" />{" "}
                    {durationText}
                  </span>
                  {tour.code && (
                  ...
                  {itinerary.map((day) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          <div className="w-9 h-9 bg-[var(--main-color)] rounded-full flex items-center justify-center text-white font-bold text-sm flex-shrink-0">
                            {day.day}
                          </div>
                          <h3 className="flex-1 text-base font-semibold text-[var(--second-color)]">
                            {day.title}
                          </h3>
                          <span
                  ...
                  <table className="w-full">
                    ...
                    <tbody>
                      {priceTable.map((row, i) => (
                        <tr
                          key={i}
                          className={`border-b border-gray-100 ${i % 2 === 0 ? "bg-white" : "bg-gray-50/60"} hover:bg-[var(--main-color)]/5 transition-colors`}
                        >
                          <td className="py-3.5 px-5 text-gray-700 text-sm">
                            {row.category}
                          </td>
                          <td className="py-3.5 px-5 text-right font-bold text-[var(--second-color)]">
                            ${row.price}
                          </td>
                        </tr>
                      ))}
                ...
                <div className="bg-white rounded-2xl p-6 md:p-8 shadow-sm border border-gray-100">
                  ...
                  <div className="flex items-center gap-3 mb-6">
                    <div
                      ...
                      style={{
                        background:
                          "linear-gradient(135deg, var(--second-color) 0%, #3d3586 100%)",
                      }}
                    >
                  ...
                  <div
                    ...
                    style={{
                      background:
                        "linear-gradient(135deg, rgba(39,34,98,0.06) 0%, rgba(227,183,94,0.10) 100%)",
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 bg-gray-50 rounded-xl px-2 py-2 border border-gray-200">
                        <button
                          type="button"
                          className="ctr-btn"
                          onClick={() => handleCounter("adults", false)}
                          disabled={formData.adults <= 1}
                        >
                          −
                        </button>
                        <span className="flex-1 text-center font-bold text-[var(--second-color)] text-sm">
                          {formData.adults}
                        </span>
                        <button
                          type="button"
                          className="ctr-btn"
                          onClick={() => handleCounter("adults", true)}
                      ...
                      {fieldErrors.adults && (
                        <p className={errCls}>{fieldErrors.adults}</p>
                      )}
                    </div>
                    <div>
                      ...
                      <div className="flex items-center gap-2 bg-gray-50 rounded-xl px-2 py-2 border border-gray-200">
                        <button
                          type="button"
                          className="ctr-btn"
                          onClick={() => handleCounter("children", false)}
                        ...
                        <button
                          type="button"
                          className="ctr-btn"
                          onClick={() => handleCounter("children", true)}
                      ...
                      {fieldErrors.children && (
                        <p className={errCls}>{fieldErrors.children}</p>
                      )}
                    </div>
                  </div>
                            ...
                            <div key={i} className="child-in">
                              <div className="relative">
                                <input
                                  ...
                                  onBlur={(e) =>
                                    handleChildAgeBlur(i, e.target.value)
                                  }
                                  className={`${inputCls(fieldErrors[`childAges_${i}`])} pl-7`}
                                  placeholder={`Child ${i + 1}`}
                                />
                                <span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-xs text-gray-400 font-semibold select-none">
                                  {i + 1}
                                </span>
                              </div>
                ...
                <div className="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-100">
                  ...
                  <div className="p-4 space-y-3">
                    {relatedArticles.map((article) => (
                      <a
                        key={article.id}
                        href={`/blogs/${article.blog_category?.slug ?? "travel"}/${article.slug}`}
        ...
        {isLightboxOpen && (
          <div className="fixed inset-0 bg-black/95 z-50 flex items-center justify-center p-4">
            <button
              ...
              className="absolute top-4 right-4 text-white hover:text-[var(--main-color)] transition-colors z-10"
            >
              <X className="w-8 h-8" />
            </button>
            <button
              onClick={() => navigateLightbox("prev")}
              className="absolute left-4 top-1/2 -translate-y-1/2 text-white hover:text-[var(--main-color)] transition-colors z-10"
            >
              <ChevronLeft className="w-12 h-12" />
            </button>
            <button
              onClick={() => navigateLightbox("next")}
              className="absolute right-4 top-1/2 -translate-y-1/2 text-white hover:text-[var(--main-color)] transition-colors z-10"
            >
